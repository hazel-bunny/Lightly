...
namespace Lightly
{
    ...
    SizeGrip::SizeGrip(Decoration *decoration)
        : QWidget(nullptr), m_decoration(decoration)
    {
    ...
    void SizeGrip::mousePressEvent(QMouseEvent *event)
    {
        switch (event->button()) {
            ...
            case Qt::LeftButton:
                if (rect().contains(event->pos())) { sendMoveResizeEvent(event->pos()); }
                break;
    ...
    void SizeGrip::sendMoveResizeEvent(QPoint position)
    {
        ...
        */
        ...
        if (reply) {
            ...
            ScopedPointer<xcb_translate_coordinates_reply_t>
                coordReply(xcb_translate_coordinates_reply(connection, coordCookie, nullptr));
