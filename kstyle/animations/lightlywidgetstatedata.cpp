...
namespace Lightly
{
    ...
    bool WidgetStateData::updateState(bool value, AnimationParameters parameters)
    {
        if (!_initialized) {
            ...
            _state = value;
            animation().data()
                ->setDirection((parameters & AnimationForwardOnly) ? Animation::Forward : _state ? Animation::Forward
                                                                                                 : Animation::Backward);
            if (parameters & AnimationOutBack) {
                animation().data()->setEasingCurve(_state ? QEasingCurve::OutBack : QEasingCurve::InQuint);
            }
            else {
                animation().data()->setEasingCurve((parameters & AnimationForwardOnly) ? QEasingCurve::OutQuint : _state
                                                                                                                  ? QEasingCurve::OutQuint
                                                                                                                  : QEasingCurve::InQuint);
            }

            if ((parameters & AnimationLongDuration)) {
                animation().data()->setDuration(
                    StyleConfigData::animationsDuration() * 3);
