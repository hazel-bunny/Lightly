...
namespace Lightly
{
    ...
    BaseEngine::WidgetList WidgetStateEngine::registeredWidgets(AnimationModes mode) const
    {
        ...
        if (mode & AnimationHover) {
                foreach(const Value &value, _hoverData) { if (value) { out.insert(value.data()->target().data()); }}
        }

        if (mode & AnimationFocus) {
                foreach(const Value &value, _focusData) { if (value) { out.insert(value.data()->target().data()); }}
        }

        if (mode & AnimationEnable) {
                foreach(const Value &value, _enableData) { if (value) { out.insert(value.data()->target().data()); }}
        }

        if (mode & AnimationPressed) {
                foreach(const Value &value, _pressedData) { if (value) { out.insert(value.data()->target().data()); }}
        }
    ...
    //____________________________________________________________
    bool WidgetStateEngine::updateState(const QObject *object,
                                        AnimationMode mode,
                                        bool value,
