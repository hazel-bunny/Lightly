        ...
        if (qApp) {
            connect(qApp, &QApplication::paletteChanged, this, [=]()
            {
                if (qApp->property("KDE_COLOR_SCHEME_PATH").isValid()) {
                    ...
                    _activeTitleBarColor = group.readEntry("activeBackground",
                                                           palette.color(QPalette::Active, QPalette::Highlight));
                    _activeTitleBarTextColor = group.readEntry("activeForeground",
                                                               palette
                                                                   .color(QPalette::Active, QPalette::HighlightedText));
                    _inactiveTitleBarColor = group.readEntry("inactiveBackground",
                                                             palette.color(QPalette::Disabled, QPalette::Highlight));
                    _inactiveTitleBarTextColor = group.readEntry("inactiveForeground",
                                                                 palette
...
//____________________________________________________________________
    void Helper::loadConfig()
    {
        ...
        _activeTitleBarColor = appGroup.readEntry("activeBackground",
                                                  globalGroup.readEntry("activeBackground",
                                                                        palette
                                                                            .color(QPalette::Active,
                                                                                   QPalette::Highlight)));
        _activeTitleBarTextColor = appGroup.readEntry("activeForeground",
                                                      globalGroup.readEntry("activeForeground",
                                                                            palette.color(QPalette::Active,
                                                                                          QPalette::HighlightedText)));
        _inactiveTitleBarColor = appGroup.readEntry("inactiveBackground",
                                                    globalGroup.readEntry("inactiveBackground",
                                                                          palette.color(QPalette::Disabled,
                                                                                        QPalette::Highlight)));
        _inactiveTitleBarTextColor = appGroup.readEntry("inactiveForeground",
                                                        globalGroup.readEntry("inactiveForeground",
                                                                              palette.color(QPalette::Disabled,
                                                                                            QPalette::HighlightedText)));
    }

//____________________________________________________________________
    QColor Helper::frameOutlineColor(const QPalette &palette,
                                     bool mouseOver,
                                     bool hasFocus,
                                     qreal opacity,
...
//____________________________________________________________________
    QColor
    Helper::sidePanelOutlineColor(const QPalette &palette, bool hasFocus, qreal opacity, AnimationMode mode) const
    {
...
//____________________________________________________________________
    QColor Helper::arrowColor(const QPalette &palette, QPalette::ColorGroup group, QPalette::ColorRole role) const
    {
        switch (role) {
            case QPalette::Text:
                return KColorUtils::mix(palette.color(group, QPalette::Text), palette.color(group, QPalette::Base),
                                        arrowShade);
...
//____________________________________________________________________
    QColor
    Helper::arrowColor(const QPalette &palette, bool mouseOver, bool hasFocus, qreal opacity, AnimationMode mode) const
    {
...
//____________________________________________________________________
    QColor Helper::buttonOutlineColor(const QPalette &palette,
                                      bool mouseOver,
                                      bool hasFocus,
                                      qreal opacity,
                                      AnimationMode mode) const
...
//____________________________________________________________________
    QColor Helper::buttonBackgroundColor(const QPalette &palette,
                                         bool mouseOver,
                                         bool hasFocus,
                                         bool sunken,
                                         qreal opacity,
    ...
    {
        QColor background(
            sunken ? KColorUtils::mix(palette.color(QPalette::Button), palette.color(QPalette::ButtonText), 0.2)
...
//____________________________________________________________________
    QColor Helper::toolButtonColor(const QPalette &palette,
                                   bool mouseOver,
                                   bool hasFocus,
                                   bool sunken,
                                   qreal opacity,
...
//____________________________________________________________________
    QColor Helper::sliderOutlineColor(const QPalette &palette,
                                      bool mouseOver,
                                      bool hasFocus,
                                      qreal opacity,
                                      AnimationMode mode) const
...
//____________________________________________________________________
    QColor Helper::scrollBarHandleColor(const QPalette &palette,
                                        bool mouseOver,
                                        bool hasFocus,
                                        qreal opacity,
                                        AnimationMode mode) const
...
//______________________________________________________________________________
    QColor Helper::checkBoxIndicatorColor(const QPalette &palette,
                                          bool mouseOver,
                                          bool active,
                                          qreal opacity,
                                          AnimationMode mode) const
...
//______________________________________________________________________________
    QPalette Helper::disabledPalette(const QPalette &source, qreal ratio) const
    {
        ...
        const QList<QPalette::ColorRole> roles = {QPalette::Background, QPalette::Highlight, QPalette::WindowText,
...
//____________________________________________________________________
    ...
    {
        if (alpha >= 0 && alpha < 1.0) { color.setAlphaF(alpha * color.alphaF()); }
        return color;
    }
...
//______________________________________________________________________________
    void Helper::renderFrame(QPainter *painter,
                             const QRect &rect,
                             const QColor &color,
                             ...
                             const bool enabled) const
    {
        ...
        QRectF frameRect(rect.adjusted(Metrics::Frame_FrameWidth, Metrics::Frame_FrameWidth, -Metrics::Frame_FrameWidth,
...
//______________________________________________________________________________
                                 ...
                                 TileSet::Tiles tiles) const
    {
        ...
        shadow.render(rect.adjusted(-params.radius, -params.radius, params.radius + params.offset.x(),
                                    params.radius + params.offset.y()), painter, tiles);
...
//______________________________________________________________________________
                                   ...
                                   const qreal opacity) const
    {
        ...
        if (mode == AnimationPressed) {
            ...
            painter->drawEllipse(frameRect.center(), initRadius + (finalRadius - initRadius) * opacity,
                                 initRadius + (finalRadius - initRadius) * opacity);
...
//______________________________________________________________________________
    void Helper::renderToolBoxFrame(QPainter *painter, const QRect &rect, int tabWidth, const QColor &outline) const
    {
        ...
        path.arcTo(QRectF(QPointF((baseRect.width() - tabWidth) / 2 - 2 * radius, baseRect.height() - 1 - 2 * radius),
                          cornerSize), 270, 90);
        ...
        path.arcTo(
            QRectF(QPointF((baseRect.width() + tabWidth) / 2 - 1, baseRect.height() - 1 - 2 * radius), cornerSize),
            180, 90);
                                      ...
                                      const int tabBarSize) const
    {
        ...
        if (tabBarSize > 0) {
            ...
            if ((tabBarCorners & CornerTopLeft) && (tabBarCorners & CornerTopRight)) {
                QRect tabBarRect = QRect(QPoint(frameRect.x() - shadowOverlap, frameRect.y() - shadowOverlap),
                                         QSize(frameRect.width() + shadowOverlap * 2, tabBarSize + 1));
        ...
        if (enabled) {
            ...
            if (hasFocus) {
                ...
                if (mode == 2 && opacity > 0 && opacity < 1) {
                    ...
                    pmask.drawEllipse(QPointF(frameRect.x(), frameRect.y() + frameRect.height() / 2), finalRadius,
                                      finalRadius);
                    pmask.end();
                    ...
                    p.setPen(Qt::NoPen);
                    renderBoxShadow(&p, frameRect, 0, 1, 6, outline.darker(120), radius,
                                    windowActive); // comment this out for only the outline animation
                    renderBoxShadow(&p, frameRect, 0, 1, 4, outline.darker(130), radius,
                                    windowActive); // comment this out for only the outline animation
                    renderBoxShadow(&p, frameRect, 0, 1, 4, outline.darker(140), radius,
                                    windowActive); // comment this out for only the outline animation
                ...
                if (mode == 2 && opacity > 0 && opacity < 1) {
                    ...
                    renderBoxShadow(painter, frameRect, 0, 1, 5, QColor(0, 0, 0, 84 * (1 - opacity)), radius,
                                    windowActive);
                    renderOutline(painter, frameRect, radius, 6 * (1 - opacity));
                    painter->setPen(Qt::NoPen);
                }
...
//______________________________________________________________________________
                                ...
                                qreal animation) const
    {
        ...
        QRectF frameRect(rect);
        frameRect.adjust(Metrics::Frame_FrameWidth - 1, Metrics::Frame_FrameWidth - 1, -Metrics::Frame_FrameWidth + 1,
        ...
        if (state == CheckOff && !isInMenu) {
            ...
            if (!isInMenu) {
                if (darkTheme) {
                    renderBoxShadow(painter, frameRect, 0, 1, 4,
                                    mouseOver ? background.darker(140) : background.darker(200), radius, windowActive);
                }
                else {
            ...
            checkShadow.cubicTo(4.67 * scale + x, 11.64 * scale + y, 5.31 * scale + x, 12.29 * scale + y,
                                5.95 * scale + x, 13.12 * scale + y);
            checkShadow.cubicTo(6.58 * scale + x, 13.95 * scale + y, 7.23 * scale + x, 14.99 * scale + y,
                                7.78 * scale + x, 14.87 * scale + y);
            checkShadow.cubicTo(8.33 * scale + x, 14.76 * scale + y, 9.3 * scale + x, 13.34 * scale + y,
                                10.52 * scale + x, 11.55 * scale + y);
            checkShadow.cubicTo(11.75 * scale + x, 9.76 * scale + y, 13.38 * scale + x, 7.38 * scale + y,
                                15.0 * scale + x, 5.0 * scale + y);
            ...
            check.cubicTo(4.67 * scale + x, 10.64 * scale + y, 5.31 * scale + x, 11.29 * scale + y, 5.95 * scale + x,
                          12.12 * scale + y);
            check.cubicTo(6.58 * scale + x, 12.95 * scale + y, 7.23 * scale + x, 13.99 * scale + y, 7.78 * scale + x,
                          13.87 * scale + y);
            check.cubicTo(8.33 * scale + x, 13.76 * scale + y, 9.3 * scale + x, 12.34 * scale + y, 10.52 * scale + x,
                          10.55 * scale + y);
            check.cubicTo(11.75 * scale + x, 8.76 * scale + y, 13.38 * scale + x, 6.38 * scale + y, 15.0 * scale + x,
                          4.0 * scale + y);
            ...
            if (animation == 0) {
                ...
                background = mouseOver ? palette.color(QPalette::Highlight).lighter(110) : palette.color(
                    QPalette::Highlight);
                ...
                checkShadow.cubicTo(animation * 4.67 + x, 11.64 + y, animation * 5.31 + x, 12.29 + y,
                                    animation * 5.95 + x, 13.12 + y);
                checkShadow.cubicTo(animation * 6.58 + x, 13.95 + y, animation * 7.23 + x, 14.99 + y,
                                    animation * 7.78 + x, 14.87 + y);
                checkShadow.cubicTo(animation * 8.33 + x, 14.76 + y, animation * 9.3 + x, 13.34 + y,
                                    animation * 10.52 + x, 11.55 + y);
                checkShadow.cubicTo(animation * 11.75 + x, 9.76 + y, animation * 13.38 + x, 7.38 + y,
                                    animation * 15.0 + x, 5.0 + y);
                ...
                check.cubicTo(animation * 4.67 + x, 10.64 + y, animation * 5.31 + x, 11.29 + y, animation * 5.95 + x,
                              12.12 + y);
                check.cubicTo(animation * 6.58 + x, 12.95 + y, animation * 7.23 + x, 13.99 + y, animation * 7.78 + x,
                              13.87 + y);
                check.cubicTo(animation * 8.33 + x, 13.76 + y, animation * 9.3 + x, 12.34 + y, animation * 10.52 + x,
                              10.55 + y);
                check.cubicTo(animation * 11.75 + x, 8.76 + y, animation * 13.38 + x, 6.38 + y, animation * 15.0 + x,
                              4.0 + y);
                                   ...
                                   qreal animation) const
    {
        ...
        if (state == RadioOn) {
            ...
            if (animation > 0 /*&& animation < 1*/ ) {
                ...
                // strong shadows don't look good with light themes
                QColor shadowColor(mouseOver ? alphaColor(background.darker(110), qMin(animation, 1.0)) : alphaColor(
                    background.darker(200), qMin(animation, 1.0)));
                if (darkTheme) {
                ...
                painter->setBrush(alphaColor(
                    (mouseOver ? palette.color(QPalette::Highlight).lighter(110) : palette.color(
                        QPalette::Highlight)), qMin(animation, 1.0)));
                painter->drawEllipse(frameRect);
...
//______________________________________________________________________________
    void
    Helper::renderDialGroove(QPainter *painter, const QRect &rect, const QColor &color, qreal first, qreal last) const
    {
        // setup painter
        painter->setRenderHint(QPainter::Antialiasing, true);
...
//______________________________________________________________________________
    void Helper::renderDialContents(QPainter *painter,
                                    const QRect &rect,
                                    const QColor &color,
                                    qreal first,
                                    qreal second) const
...
//______________________________________________________________________________
    void Helper::renderSliderHandle(QPainter *painter,
                                    const QRect &rect,
                                    const QColor &color,
                                    const bool focus,
                                    bool sunken) const
                                               ...
                                               int progress) const
    {
        ...
        if (horizontal) {
            ...
            if (progress > Metrics::ProgressBar_BusyIndicatorSize) {
                painter.drawRect(QRect(0, 0, Metrics::ProgressBar_BusyIndicatorSize, 1).translated(
                    progress - 2 * Metrics::ProgressBar_BusyIndicatorSize, 0));
            }
        }
        else {
            QPainter painter(&pixmap);
            ...
            if (progress > Metrics::ProgressBar_BusyIndicatorSize) {
                painter.drawRect(QRect(0, 0, 1, Metrics::ProgressBar_BusyIndicatorSize).translated(0,
                                                                                                   progress - 2
                                                                                                       * Metrics::ProgressBar_BusyIndicatorSize));
            }
        }
...
//______________________________________________________________________________
    void Helper::renderScrollBarHandle(QPainter *painter, const QRect &rect, const QColor &color) const
    {
        ...
        const QRectF baseRect(rect);
        const qreal radius(
            0.5 * std::min({baseRect.width(), baseRect.height(), (qreal) Metrics::ScrollBar_SliderWidth}));
...
//______________________________________________________________________________
    void
    Helper::renderArrow(QPainter *painter, const QRect &rect, const QColor &color, ArrowOrientation orientation) const
    {
        // define polygon
        QPolygonF arrow;
        switch (orientation) {
                                        ...
                                        bool inverted) const
    {
        ...
        switch (buttonType) {
            ...
            case ButtonRestore: {
                ...
                painter->setPen(pen);
                painter->drawPolygon(
                    QVector<QPointF>{QPointF(4.5, 9), QPointF(9, 4.5), QPointF(13.5, 9), QPointF(9, 13.5)});
                break;
            }
...
//________________________________________________________________________________________________________
    ...
    {
#if LIGHTLY_HAVE_X11
        if (isX11()) { return QX11Info::isCompositingManagerRunning(QX11Info::appScreen()); }
#endif
...
//______________________________________________________________________________________
    ...
    QPixmap Helper::coloredIcon(const QIcon &icon,
                                const QPalette &palette,
                                const QSize &size,
                                QIcon::Mode mode,
                                QIcon::State state)
