...
namespace Lightly
{
    ...
    void Style::polish(QWidget *widget)
    {
        ...
        if (auto itemView = qobject_cast<QAbstractItemView *>(widget)) {
            ...
            if (!StyleConfigData::titleWidgetDrawFrame()) { widget->setBackgroundRole(QPalette::Window); }
        }
        ...
        if (qobject_cast<QScrollBar *>(widget)) {
            ...
            // and alter palette
            widget->setAutoFillBackground(false);
            widget->setContentsMargins(Metrics::Frame_FrameWidth,
    ...
    void Style::polishScrollArea(QAbstractScrollArea *scrollArea)
    {
        ...
        if (scrollArea->property(PropertyNames::sidePanelView).toBool()) {
            ...
            QWidget *viewPort =
                scrollArea->findChild<QWidget *>(QString("qt_scrollarea_viewport"), Qt::FindDirectChildrenOnly);
        ...
        if (!(scrollArea->frameShape() == QFrame::NoFrame
            || scrollArea->backgroundRole() == QPalette::Window)) { return; }
    ...
    void Style::unpolish(QWidget *widget)
    {
        ...
        if (qobject_cast<QAbstractScrollArea *>(widget) ||
            ...
            widget->inherits("QComboBoxPrivateContainer")) { widget->removeEventFilter(this); }
    ...
    int Style::pixelMetric(PixelMetric metric, const QStyleOption *option, const QWidget *widget) const
    {
        ...
        switch (metric) {
            ...
            case PM_DefaultFrameWidth:
                ...
                else if (widget && widget->inherits("KTextEditor::View") && !StyleConfigData::kTextEditDrawFrame()
                    && !_isKdevelop) {
                    return 0;
                ...
                if (auto mask = qstyleoption_cast<QStyleHintReturnMask *>(returnData)) {
                    ...
                    if (widget && (
                        qobject_cast<const QAbstractItemView *>(widget->parent()) ||
                            ...
                            qobject_cast<const QMainWindow *>(widget->parent()))) { return true; }

                    // also check if widget's parent is some itemView viewport
                    if (widget && widget->parent() &&
                        qobject_cast<const QAbstractItemView *>(widget->parent()->parent()) &&
                        static_cast<const QAbstractItemView *>( widget->parent()->parent())->viewport()
                            == widget->parent()) { return true; }
    ...
    //______________________________________________________________
    QSize Style::sizeFromContents(ContentsType element,
                                  const QStyleOption *option,
                                  const QSize &size,
                                  const QWidget *widget) const
    {
                              ...
                              const QWidget *widget) const
    {
        ...
        if (!(fcn && fcn(*this, option, painter, widget))) {
            ParentStyleClass::drawPrimitive(element,
                                            option,
                                            painter,
                                            widget);
        }
                            ...
                            const QWidget *widget) const
    {
        ...
        // call function if implemented
        if (!(fcn && fcn(*this, option, painter, widget))) {
            ParentStyleClass::drawControl(element,
                                          option,
                                          painter,
                                          widget);
        }
                                   ...
                                   const QWidget *widget) const
    {
        ...
        // call function if implemented
        if (!(fcn && fcn(*this, option, painter, widget))) {
            ParentStyleClass::drawComplexControl(element,
                                                 option,
                                                 painter,
                                                 widget);
        }
        ...
        const QString &text, QPalette::ColorRole textRole) const
    {
        ...
        if (_animations->widgetEnabilityEngine().enabled()) {
            ...
            if (_animations->widgetEnabilityEngine().isAnimated(widget, AnimationEnable)) {
                ...
                const QPalette copy(_helper->disabledPalette(palette,
                                                             _animations->widgetEnabilityEngine()
                                                                 .opacity(widget, AnimationEnable)));
    ...
    bool Style::eventFilter(QObject *object, QEvent *event)
    {
        ...
        if (auto dockWidget = qobject_cast<QDockWidget *>(object)) { return eventFilterDockWidget(dockWidget, event); }
            ...
            return eventFilterMdiSubWindow(subWindow,
                                           event);
        }
        else if (auto commandLinkButton = qobject_cast<QCommandLinkButton *>(object)) {
            return eventFilterCommandLinkButton(commandLinkButton,
                                                event);
        }
#if QT_VERSION < 0x050D00 // Check if Qt version < 5.13
        ...
        if (widget->inherits("QAbstractScrollArea")
            || widget->inherits("KTextEditor::View")) { return eventFilterScrollArea(widget, event); }
    ...
    bool Style::eventFilterScrollArea(QWidget *widget, QEvent *event)
    {
        ...
        switch (event->type()) {   // TODO: delete
            case QEvent::Paint: {
                ...
                if ((child = scrollArea->findChild<QWidget *>("qt_scrollarea_hcontainer"))
                    && child->isVisible()) { children.append(child); }
    ...
    bool Style::eventFilterDockWidget(QDockWidget *dockWidget, QEvent *event)
    {
        if (event->type() == QEvent::Paint) {
            ...
            if (dockWidget->isFloating()) {
                ...
                || (dockWidget->features() & QDockWidget::AllDockWidgetFeatures)) {
                ...
                if (_isDolphin && dockWidget->inherits("DolphinDockWidget")
                    ...
                    if (StyleConfigData::dolphinSidebarOpacity()
                        < (palette.color(QPalette::Window).alpha() / 255) * 100) {
                        /*painter.setBrush( Qt::NoBrush );
                        ...
                        const QWidget *tabWidget =
                            dockWidget->window()->findChild<const QWidget *>("tabWidget", Qt::FindDirectChildrenOnly);
                        if (tabWidget) {
            ...
            if (dockWidget->inherits("DolphinDockWidget") && _isDolphin
    ...
    bool Style::eventFilterCommandLinkButton(QCommandLinkButton *button, QEvent *event)
    {
        ...
        if (event->type() == QEvent::Paint) {
            ...
            if (!button->icon().isNull()) {
                ...
                const QRect pixmapRect(QPoint(offset.x(),
                                              button->description().isEmpty() ? (button->height() - pixmapSize.height())
                                                  / 2 : offset.y()), pixmapSize);
            ...
            // text rect
            QRect textRect
                (offset, QSize(button->size().width() - offset.x() - margin, button->size().height() - 2 * margin));
            const QPalette::ColorRole textRole =
                (enabled && hasFocus && !mouseOver && !isFlat) ? QPalette::HighlightedText : QPalette::ButtonText;
    ...
    QRect Style::checkBoxContentsRect(const QStyleOption *option, const QWidget *) const
    {
        return visualRect(option,
                          option->rect.adjusted(Metrics::CheckBox_Size + Metrics::CheckBox_ItemSpacing, 0, 0, 0));
    }
    ...
    QRect Style::progressBarContentsRect(const QStyleOption *option, const QWidget *widget) const
    {
        ...
        if (horizontal) {
            ...
            indicatorRect = QRect(inverted ? (rect.right() - indicatorSize + 1) : rect.left(),
                                  rect.y(),
                                  indicatorSize,
                                  rect.height());
            ...
            indicatorRect = QRect(rect.x(),
                                  inverted ? rect.top() : (rect.bottom() - indicatorSize + 1),
                                  rect.width(),
                                  indicatorSize);
        }
                                          ...
                                          const QWidget *widget) const
    {
        ...
        switch (subControl) {
            case SC_ToolButtonMenu: {
                ...
                if (hasInlineIndicator) { menuRect.setTop(menuRect.bottom() - menuButtonWidth + 1); }
                                        ...
                                        const QWidget *widget) const
    {
        ...
        switch (subControl) {
            ...
            case SC_ComboBoxArrow: {
                ...
                arrowRect =
                    centerRect(arrowRect, Metrics::MenuButton_IndicatorWidth, Metrics::MenuButton_IndicatorWidth);
                return visualRect(option, arrowRect);
            }
        ...
        switch (subControl) {
            ...
            case SC_ScrollBarAddLine: {
                ...
                if (horizontal) {
                    ...
                    return visualRect(option,
                                      QRect(rect.left(), rect.bottom() - majorSize + 1, rect.width(), majorSize));
                }
            }
        ...
        switch (subControl) {
            ...
            case SC_ScrollBarSlider: {
                ...
                int sliderSize = space * qreal(sliderOption->pageStep)
                    / (sliderOption->maximum - sliderOption->minimum + sliderOption->pageStep);
                ...
                int pos = qRound(qreal(sliderOption->sliderPosition - sliderOption->minimum)
                                     / (sliderOption->maximum - sliderOption->minimum) * space);
                if (sliderOption->upsideDown) { pos = space - pos; }
                if (horizontal) {
                    return visualRect(option,
                                      QRect(groove.left() + pos,
                                            groove.top(),
                                            sliderSize,
                                            groove.height()));
                }
                    ...
                    return visualRect(option,
                                      QRect(groove.left(), groove.top() + pos, groove.width(), sliderSize));
                }
            }
            ...
            case SC_ScrollBarSubPage: {
                ...
                if (horizontal) {
                    ...
                    return visualRect(option,
                                      QRect(groove.left(), groove.top(), groove.width(), slider.top() - groove.top()));
                }
            }
                                            ...
                                            const QWidget *widget) const
    {
        ...
        if (hasText) { size.setWidth(qMax(size.width(), int(Metrics::Button_MinWidth))); }
                                            ...
                                            const QWidget *) const
    {
        ...
        const int marginWidth
            (autoRaise ? Metrics::ToolButton_MarginWidth : Metrics::Button_MarginWidth + Metrics::Frame_FrameWidth);
        ...
        switch (menuItemOption->menuItemType) {
            ...
            case QStyleOptionMenuItem::SubMenu: {
                ...
                if (showIconsInMenuItems()) {
                    iconWidth = isQtQuickControl(option, widget) ? qMax(pixelMetric(PM_SmallIconSize,
                                                                                    option,
                                                                                    widget),
                                                                        menuItemOption->maxIconWidth) : menuItemOption
                                    ->maxIconWidth;
                }
                ...
                // add icon width
                if (iconWidth > 0) { leftColumnWidth += iconWidth + Metrics::MenuItem_ItemSpacing; }
                ...
                if (menuItemOption->text.isEmpty() && menuItemOption->icon.isNull()) {
                    ...
                    if (!menuItemOption->text.isEmpty()) {
                        ...
                        size.setWidth(qMax(size.width(),
                                           menuItemOption->fontMetrics.boundingRect(menuItemOption->text).width()));
                    }
                                           ...
                                           const QWidget *widget) const
    {
        ...
        if (verticalTabs) {
            ...
            if (contentsSize.height() == tabBarHeight && tabBarHeight + 2 * (Metrics::Frame_FrameWidth - 1)
        ...
        if (verticalTabs) {
            ...
            if (hasIcon && !hasText) { size = size.expandedTo(QSize(0, Metrics::TabBar_TabMinHeight)); }
                ...
                size = size.expandedTo(QSize(Metrics::TabBar_TabMinWidth,
                                             Metrics::TabBar_TabMinHeight + (documentMode ? 0 : 2 * 4)));
            }
        }
    ...
    bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        const bool isInputWidget((widget && widget->testAttribute(Qt::WA_Hover)) ||
            (isQtQuickControl(option, widget)
                && option->styleObject->property("elementType").toString() == QStringLiteral("edit")));
        ...
        if (!StyleConfigData::sidePanelDrawFrame() && widget
            ...
            const auto
                background(isTitleWidget ? palette.color(widget->backgroundRole()) : palette.color(QPalette::Base));
    ...
    bool Style::drawFrameLineEditPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (rect.height() < 2 * Metrics::LineEdit_FrameWidth + option->fontMetrics.height()) {
            ...
            _helper->renderLineEdit(painter,
                                    rect,
                                    background,
                                    outline,
                                    hasFocus,
    ...
    bool Style::drawFrameFocusRectPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (qobject_cast<const QAbstractButton *>(widget) || qobject_cast<const QScrollBar *>(widget)
            || qobject_cast<const QGroupBox *>(widget)) { return true; }

        // no focus indicator on ComboBox list items
        if (widget && widget->inherits("QComboBoxListView")) { return true; }

        if (option->styleObject
            && option->styleObject->property("elementType") == QLatin1String("button")) { return true; }

        const State &state(option->state);

        // no focus indicator on selected list items
        if ((state & State_Selected) && qobject_cast<const QAbstractItemView *>(widget)) { return true; }
        ...
        const auto outlineColor
            (state & State_Selected ? palette.color(QPalette::HighlightedText) : palette.color(QPalette::Highlight));
        painter->setRenderHint(QPainter::Antialiasing, false);
        ...
        if (inTabBar) {
            ...
            if (flat && hasMenu) {
                ...
                if (sunken && !mouseOver) { color = palette.color(QPalette::HighlightedText); }
                    ...
                    color = _helper
                        ->arrowColor(palette, arrowHover, false, opacity, animated ? AnimationHover : AnimationNone);
                }
            }
            else if (flat) {
    ...
    bool Style::drawIndicatorHeaderArrowPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
    {
        ...
        if (state & State_UpArrow || (headerOption && headerOption->sortIndicator == QStyleOptionHeader::SortUp)) {
            ...
            || (headerOption && headerOption->sortIndicator == QStyleOptionHeader::SortDown)) {
                                                ...
                                                const QWidget *widget) const
    {
        ...
        _animations->widgetStateEngine()
            .updateState(widget, AnimationPressed, sunken, AnimationForwardOnly | AnimationLongDuration);
        ...
        if (flat) {
            ...
            // render
            _helper->renderButtonFrame(painter,
                                       rect,
                                       background,
                                       palette,
                                       hasFocus,
                                                   ...
                                                   const QWidget *widget) const
    {
        ...
        const auto color =
            hasAlteredBackground(parent) ? _helper->frameBackgroundColor(palette) : palette.color(QPalette::Window);
    ...
    bool Style::drawPanelTipLabelPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (widget && widget->window()) { _shadowHelper->registerWidget(widget->window(), true); }
                                               ...
                                               const QWidget *widget) const
    {
        ...
        // store flags
        const State &state(option->state);
        const bool mouseOver((state & State_MouseOver) && (!abstractItemView
        ...
        const bool
            hasCustomBackground = viewItemOption->backgroundBrush.style() != Qt::NoBrush && !(state & State_Selected);
        const bool
            hasSolidBackground = !hasCustomBackground || viewItemOption->backgroundBrush.style() == Qt::SolidPattern;
        ...
        if (!(mouseOver || selected || hasCustomBackground || hasAlternateBackground)) { return true; }
        ...
        if (!(mouseOver || selected || hasCustomBackground)) { return true; }
                                               ...
                                               const QWidget *widget) const
    {
        ...
        _helper
            ->renderCheckBox(painter, rect, palette, false, false, sunken, mouseOver, checkBoxState, false, animation);
        return true;
    }
                                                  ...
                                                  const QWidget *widget) const
    {
        ...
        _animations->widgetStateEngine().updateState(widget, AnimationHover, mouseOver);
        _animations->widgetStateEngine()
            .updateState(widget, AnimationPressed, radioButtonState != RadioOff, AnimationOutBack);
        ...
        _helper
            ->renderRadioButton(painter, rect, palette, mouseOver, sunken, radioButtonState, false, false, animation);
    ...
    //___________________________________________________________________________________
    bool Style::drawIndicatorToolBarSeparatorPrimitive(const QStyleOption *option,
                                                       QPainter *painter,
                                                       const QWidget *widget) const
    {
         ...
         */
        if (!(StyleConfigData::toolBarDrawItemSeparator() || qobject_cast<const QComboBox *>(widget))) { return true; }
    ...
    bool Style::drawIndicatorBranchPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *) const
    {
        ...
        if (state & State_Children) {
            ...
            // color
            const auto
                arrowColor(mouseOver ? _helper->hoverColor(palette) : _helper->arrowColor(palette, QPalette::Text));
        ...
        if (state & State_Item) {
            const QLineF line = reverseLayout ?
                                QLineF(QPointF(rect.left(), center.y()),
    ...
    bool Style::drawPushButtonLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (hasIcon && iconRect.isValid()) {
            ...
            const auto
                pixmap = _helper->coloredIcon(buttonOption->icon, buttonOption->palette, iconSize, iconMode, iconState);
    ...
    bool Style::drawToolButtonLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (hasText && (!(hasArrow || hasIcon) || toolButtonOption->toolButtonStyle == Qt::ToolButtonTextOnly)) {
            ...
            const bool
                leftAlign(widget && widget->property(PropertyNames::toolButtonAlignment).toInt() == Qt::AlignLeft);
            if (leftAlign) {
                const int marginWidth(Metrics::Button_MarginWidth + Metrics::Frame_FrameWidth + 1);
                iconRect = QRect(QPoint(contentsRect.left() + marginWidth,
        ...
        if (hasArrow && iconRect.isValid()) {
            ...
            const QPixmap pixmap =
                _helper->coloredIcon(toolButtonOption->icon, toolButtonOption->palette, iconSize, iconMode, iconState);
            drawItemPixmap(painter, iconRect, Qt::AlignCenter, pixmap);
        }
        ...
        if (hasText && textRect.isValid()) {
            ...
            if (flat) {
                textRole = (((hasFocus && sunken) || (state & State_Sunken)) && !mouseOver) ? QPalette::HighlightedText
                                                                                            : QPalette::WindowText;
            }
    ...
    bool Style::drawCheckBoxLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        const bool reverseLayout(option->direction == Qt::RightToLeft);
        const int
            textFlags(_mnemonics->textFlags() | Qt::AlignVCenter | (reverseLayout ? Qt::AlignRight : Qt::AlignLeft));
        ...
        if (!buttonOption->icon.isNull()) {
            ...
            const QPixmap
                pixmap(_helper->coloredIcon(buttonOption->icon, buttonOption->palette, buttonOption->iconSize, mode));
    ...
    bool Style::drawComboBoxLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (sunken && !flat) { painter->translate(1, 1); }
    ...
    bool Style::drawMenuBarEmptyAreaControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (_helper->titleBarColor(true).alphaF() == 1
            || !_translucentWidgets.contains(widget->window())) { return true; }
    ...
    bool Style::drawMenuBarItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (widget && _helper->titleBarColor(true).alphaF() * 100.0 < 100
            && _translucentWidgets.contains(widget->window()) && StyleConfigData::widgetDrawShadow()) {
        ...
        */
        if (!menuItemOption->icon.isNull()) {
            ...
            const auto pixmap = _helper
                ->coloredIcon(menuItemOption->icon, menuItemOption->palette, iconRect.size(), iconMode, iconState);
            drawItemPixmap(painter, iconRect, Qt::AlignCenter, pixmap);
            ...
            const QPalette::ColorRole
                role = (useStrongFocus && sunken) ? QPalette::HighlightedText : QPalette::WindowText;
    ...
    bool Style::drawMenuItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (useStrongFocus && (selected || sunken)) {
            ...
            if (animatedRect.isNull()) {
                ...
                if (animated && intersected) {
                    painter->drawRoundedRect(copy.adjusted(3 * (1 - opacity),
                                                           3 * (1 - opacity),
                                                           -3 * (1 - opacity),
                                                           -3 * (1 - opacity)),
        ...
        if (menuItemOption->checkType == QStyleOptionMenuItem::NonExclusive) {
            ...
            const auto
                background(state == CheckOn ? palette.color(QPalette::Highlight) : palette.color(QPalette::Button));
        ...
        if (showIcon) {
            iconWidth = isQtQuickControl(option, widget) ? qMax(pixelMetric(PM_SmallIconSize, option, widget),
                                                                menuItemOption->maxIconWidth) : menuItemOption
                            ->maxIconWidth;
        }
        ...
        if (showIcon && iconWidth > 0) {
            iconRect = QRect(contentsRect.left(),
                             contentsRect.top() + (contentsRect.height() - iconWidth) / 2,
                             iconWidth,
                             iconWidth);
            contentsRect.setLeft(iconRect.right() + Metrics::MenuItem_ItemSpacing * 2);
            const QSize
                iconSize(pixelMetric(PM_SmallIconSize, option, widget), pixelMetric(PM_SmallIconSize, option, widget));
        ...
        if (showIcon && !menuItemOption->icon.isNull()) {
            ...
            const QIcon::State iconState(sunken ? QIcon::On : QIcon::Off);
            const QPixmap icon =
                _helper->coloredIcon(menuItemOption->icon, menuItemOption->palette, iconRect.size(), mode, iconState);
        ...
        if (!menuItemOption->text.isEmpty()) {
            ...
            // render text
            const int textFlags
                (Qt::AlignVCenter | (reverseLayout ? Qt::AlignRight : Qt::AlignLeft) | _mnemonics->textFlags());
    ...
    bool Style::drawToolBarBackgroundControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        else if ((_helper->titleBarColor(true).alphaF() * 100.0 == 100
            && widget->window()->palette().color(QPalette::Window).alpha() == 255)
        ...
        if (option->state & State_Horizontal) {
            ...
            if (bellowMenuBar || widget->y() == 0) {
                ...
                if (_isDolphin && StyleConfigData::dolphinSidebarOpacity() < 100) {
                    QList<QWidget *> sidebars = widget->window()
                        ->findChildren<QWidget *>(QRegularExpression("^(places|terminal|info|folders)Dock$"),
                ...
                if (!darkTheme) {
                    int shadowSize = 4;
                    QRect shadowRect = QRect(copy.bottomLeft() - QPoint(shadowSize, -1),
    ...
    bool Style::drawProgressBarControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (styleObject && _animations->busyIndicatorEngine().enabled()) {
            ...
            _animations->busyIndicatorEngine()
                .setAnimated(styleObject, progressBarOption->maximum == 0 && progressBarOption->minimum == 0);
        }
    ...
    bool Style::drawProgressBarContentsControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
    {
        ...
        if (busy) {
            ...
            const auto
                second(KColorUtils::mix(palette.color(QPalette::Highlight), palette.color(QPalette::Window), 0.7));
    ...
    bool Style::drawProgressBarLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
    {
        ...
        // define text rect
        Qt::Alignment hAlign
            ((progressBarOption->textAlignment == Qt::AlignLeft) ? Qt::AlignHCenter : progressBarOption->textAlignment);
        drawItemText(painter,
                     rect,
                     Qt::AlignVCenter | hAlign,
                     palette,
                     enabled,
    ...
    bool Style::drawHeaderSectionControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (isFirst && horizontal) {
            ...
            painter->setBrush(color);
            painter->setPen(Qt::NoPen);
            painter->drawRoundedRect(QRect(rect.topLeft().x(), rect.topLeft().y(), radius + 2, radius + 2),
                                     radius + 1,
                                     radius + 1);
    ...
    bool Style::drawTabBarTabLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (tabOption) {
            ...
            if (tabOption->documentMode) {
                ...
                tr = subElementRect(SE_TabBarTabText,
                                    option,
        ...
        const bool animated
            (enabled && selected && _animations->tabBarEngine().isAnimated(widget, rect.topLeft(), AnimationFocus));
    ...
    bool Style::drawTabBarTabShapeControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        const bool animated
            (enabled && !selected && _animations->tabBarEngine().isAnimated(widget, rect.topLeft(), AnimationHover));
        ...
        const auto tabWidget =
            (widget && widget->parentWidget()) ? qobject_cast<const QTabWidget *>(widget->parentWidget()) : nullptr;
        ...
        if (selected) {
            color =
                (documentMode && !isQtQuickControl && !hasAlteredBackground(widget)) ? palette.color(QPalette::Window)
                                                                                     : _helper
                    ->frameBackgroundColor(palette);
        }
        ...
        if (selected) {
            ...
            if (documentMode) {
                ...
                painter->setBrush(color);
                painter
                    ->drawRoundedRect(backgroundRect, StyleConfigData::cornerRadius(), StyleConfigData::cornerRadius());
    ...
    bool Style::drawToolBoxTabLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (!toolBoxOption->icon.isNull()) {
            ...
            const QIcon::Mode mode(enabled ? QIcon::Normal : QIcon::Disabled);
            const QPixmap
                pixmap(_helper->coloredIcon(toolBoxOption->icon, toolBoxOption->palette, iconRect.size(), mode));
    ...
    bool Style::drawToolBoxTabShapeControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        if (selected) { outline = _helper->focusColor(palette); }
            ...
            outline = _helper
                ->frameOutlineColor(palette, mouseOver, false, opacity, isAnimated ? AnimationHover : AnimationNone);
        }
    ...
    bool Style::drawDockWidgetTitleControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
    {
        ...
        const auto buttonRect
            (subElementRect(dockWidgetOption->floatable ? SE_DockWidgetFloatButton : SE_DockWidgetCloseButton,
                            option,
                            widget));
                                           ...
                                           const QWidget *widget) const
    {
        ...
        if (!(option->subControls & QStyle::SC_GroupBoxLabel) || groupBoxOption->text.isEmpty()) { return true; }
        ...
        if (option->subControls & SC_ComboBoxFrame) {
            ...
            if (editable) {
                ...
                if (flat) {
                    ...
                    //const auto outline( _helper->buttonOutlineColor( palette, mouseOver, hasFocus, opacity, mode ) );
                    const auto
                        background(_helper->buttonBackgroundColor(palette, mouseOver, hasFocus, false, opacity, mode));
        ...
        if (option->subControls & SC_ComboBoxArrow) {
            ...
            if (editable) {
                ...
                if (empty || !enabled) { arrowColor = palette.color(QPalette::Disabled, QPalette::Text); }
                    ...
                    const bool
                        subControlHover(enabled && mouseOver && comboBoxOption->activeSubControls & SC_ComboBoxArrow);
                                          ...
                                          const QWidget *widget) const
    {
        ...
        if (option->subControls & SC_SpinBoxDown) {
            renderSpinBoxArrow(SC_SpinBoxDown,
                               spinBoxOption,
                               painter,
                               widget);
        }
                                         ...
                                         const QWidget *widget) const
    {
        ...
        if (sliderOption->subControls & SC_SliderTickmarks) {
            ...
            if (interval >= 1) {
                ...
                while (current <= sliderOption->maximum) {
                    ...
                    int position
                        (sliderPositionFromValue(sliderOption->minimum, sliderOption->maximum, current, available)
                             + fudge);
                        foreach(const QLine &tickLine, tickLines) {
                            if (horizontal) {
                                painter->drawLine(tickLine.translated(upsideDown ? (rect.width() - position) : position,
                                                                      0));
                            }
                            else {
                                painter->drawLine(tickLine.translated(0,
                                                                      upsideDown ? (rect.height() - position)
                                                                                 : position));
                            }
                        }
        ...
        if (sliderOption->subControls & SC_SliderHandle) {
            ...
            // define colors
            const auto &background = (hasFocus || mouseOver) ? palette.color(QPalette::Highlight).lighter(105) : palette
                .color(QPalette::Button).lighter(110);
                                       ...
                                       const QWidget *widget) const
    {
        ...
        if (sliderOption->subControls & SC_DialGroove) {
            ...
            const auto grooveColor
                (KColorUtils::mix(palette.color(QPalette::Window), palette.color(QPalette::WindowText), 0.3));
                                            ...
                                            const QWidget *widget) const
    {
        ...
        const bool animated(StyleConfigData::animationsEnabled() && _animations->scrollBarEngine()
            .isAnimated(widget, AnimationHover, QStyle::SC_ScrollBarGroove));
                                           ...
                                           const QWidget *widget) const
    {
        ...
        if (titleBarOption->subControls & SC_TitleBarLabel) {
            ...
            ParentStyleClass::drawItemText(painter,
                                           textRect,
                                           Qt::AlignCenter,
                                           palette,
                                           active,
    ...
    void Style::renderMenuTitle(const QStyleOptionToolButton *option, QPainter *painter, const QWidget *) const
    {
        ...
        const auto color(_helper->separatorColor(palette));
        _helper->renderSeparator(painter,
                                 QRect(option->rect.bottomLeft() - QPoint(0, Metrics::MenuItem_MarginHeight),
        ...
        const auto
            contentsRect = insideMargin(option->rect, Metrics::MenuItem_MarginWidth, Metrics::MenuItem_MarginHeight);
    ...
    //____________________________________________________________________________________
    QStyleOptionToolButton Style::separatorMenuItemOption(const QStyleOptionMenuItem *menuItemOption,
                                                          const QWidget *widget) const
    {
                                      ...
                                      const QWidget *widget) const
    {
        ...
        const ArrowOrientation
            orientation(standardPixmap == SP_ToolBarHorizontalExtensionButton ? ArrowRight : ArrowDown);
        ...
        QIcon icon;
            foreach(const IconData &iconData, iconTypes) {
                    ...
                    foreach(const int &iconSize, iconSizes) {
                        ...
                        painter.translate(
                            standardPixmap == SP_ToolBarHorizontalExtensionButton ? QPoint(1, 0) : QPoint(0, 1));
        ...
        const QList<IconData> iconTypes =
            {
                ...
                // state off icons
                {KColorUtils::mix(palette.color(QPalette::Window), base, 0.5), invertNormalState, QIcon::Normal,
                 QIcon::Off},
                {KColorUtils::mix(palette.color(QPalette::Window), selected, 0.5), invertNormalState, QIcon::Selected,
                 QIcon::Off},
                {KColorUtils::mix(palette.color(QPalette::Window), negative, 0.5), true, QIcon::Active, QIcon::Off},
                {KColorUtils::mix(palette.color(QPalette::Window), base, 0.2), invertNormalState, QIcon::Disabled,
                 QIcon::Off},

                // state on icons
                {KColorUtils::mix(palette.color(QPalette::Window), negative, 0.7), true, QIcon::Normal, QIcon::On},
                {KColorUtils::mix(palette.color(QPalette::Window), negativeSelected, 0.7), true, QIcon::Selected,
                 QIcon::On},
    ...
    const QAbstractItemView *Style::itemViewParent(const QWidget *widget) const
    {
        ...
        if ((itemView = qobject_cast<const QAbstractItemView *>(widget))) {
            ...
            widget &&
                ...
                itemView->viewport() == widget->parentWidget()) { return itemView; }
...
#if LIGHTLY_HAVE_QTQUICK
        const bool
            is = (widget == nullptr) && option && option->styleObject && option->styleObject->inherits("QQuickItem");
    ...
    */
    void Style::setSurfaceFormat(QWidget *widget) const
    {
        ...
        if (widget->inherits("QTipLabel")) {
            ...
            if (_isPlasma || _isOpaque || !widget->isWindow()
                || !_helper->shouldWindowHaveAlpha(widget->palette(), _isDolphin)) {
                return;
            }
        ...
        if (QMainWindow *mw = qobject_cast<QMainWindow *>(p)) {
            ...
            if (tb->orientation() == Qt::Vertical) {
                if (tb->y() == 0) {
                    ...
                    else if (LightlyPrivate::possibleTranslucentToolBars.contains(w)
    ...
    void Style::tabLayout(const QStyleOptionTab *opt, const QWidget *widget, QRect *textRect, QRect *iconRect) const
    {
        ...
        } // 0, 0 as we will have a translate transform
        ...
        if (!opt->icon.isNull()) {
            ...
            tabIconSize =
                QSize(qMin(tabIconSize.width(), iconSize.width()), qMin(tabIconSize.height(), iconSize.height()));
